...
def fake_parallelize(target_method: Callable, parameters: list[dict[str, Any]], num_workers: int = 0, chunksize: int = 1) -> list[T]:
    return [target_method(**t) for t in parameters]


def thread_pool_parallelize(target_method: Callable, parameters: list[dict[str, Any]], num_workers: int = 0, chunksize: int = 1) -> list[T]:
    """
    ...
    with ThreadPoolExecutor(max_workers=number_of_processes) as executor:
        map_function: Callable = partial(executor.map, chunksize=chunksize)
...
def process_pool_parallelize(target_method: Callable, parameters: list[dict[str, Any]], num_workers: int = 0, chunksize: int = 1) -> list[T]:
    """
